#!/usr/bin/env python3
"""Monitor the MoNuSeg task/spatial residual W&B batch and write a live ledger."""
...
LEDGER_PATH = Path("reports/monuseg_task_spatial_residual_live_ledger.md")
...
def _write_ledger(group: str, runs: list[Any]) -> None:
    ...
    lines = [
        "# MonuSeg Task/Spatial Residual Live Ledger",
        "",
        f"- Group: `{group}`",
        f"- Updated: `{now}`",
        f"- Runs seen: `{len(runs)}`",
        "",
        "| run | state | epoch | final Dice | final IoU | best Dice | best IoU | best epoch | alpha | W&B URL |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--group", default="monuseg_d0fpn_task_spatial_residual_aug80")
